...
using Northwind.Services.Ado.Repositories;
using Northwind.Services.Repositories;
using NUnit.Framework;

namespace Northwind.Services.Ado.Tests.Repositories
{
    [TestFixture]
    public sealed class OrderRepositoryReadTests : IDisposable
    {
        ...
        private NorthwindDatabaseService databaseService = default!;
        ...
        {
            // Act
            IList<Order> orders = await this.repository.GetOrdersAsync(skip: skip, count: count);
        ...
        {
            _ = Assert.ThrowsAsync<RepositoryException>(async () => await this.repository.GetOrderAsync(orderId: 0));
        }

        [TestCaseSource(nameof(GetOrderDataAsync))]
        public async Task GetOrderAsync_OrderIsExist_ReturnsOrder(Order expectedOrder)
        {
            // Act
            Order? actualOrder = await this.repository.GetOrderAsync(orderId: expectedOrder.Id);
        ...
        public void Init()
        {
            this.databaseService = new NorthwindDatabaseService();
            this.databaseService.CreateTables();
            this.databaseService.InitializeTables();
            this.databaseService.InitializeOrders();
            this.repository = new OrderRepository(SqliteFactory.Instance, NorthwindDatabaseService.ConnectionString);
        }
        ...
        private static IEnumerable<Order> GetOrderDataAsync()
        {
