...
namespace Northwind.Services.Ado.Tests.Repositories
{
    ...
    public sealed class OrderRepositoryTests : RepositoryTestsBase
    {
        private static readonly object[][] ConstructorData =
        {
            new object[]
            {
                new[] { typeof(DbProviderFactory), typeof(string) },
            },
        };
        ...
        public void SetUp()
        {
            this.ClassType = typeof(Ado.Repositories.OrderRepository);
        }
        ...
        [TestCase("GetOrdersAsync", false, true, true, typeof(Task<IList<Order>>))]
        [TestCase("GetOrderAsync", false, true, true, typeof(Task<Order>))]
