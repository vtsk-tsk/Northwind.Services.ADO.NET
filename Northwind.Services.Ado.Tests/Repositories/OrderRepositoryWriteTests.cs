...
namespace Northwind.Services.Ado.Tests.Repositories
{
    ...
    public class OrderRepositoryWriteTests
    {
        ...
        {
            // Arrange
            using var databaseService = new NorthwindDatabaseService();
            databaseService.CreateTables();
            databaseService.InitializeTables();
            databaseService.InitializeOrders();
            var repository = new OrderRepository(SqliteFactory.Instance, NorthwindDatabaseService.ConnectionString);
        ...
        [TestCase(10247)]
        [TestCase(11078)]
        public async Task RemoveOrderAsync_OrderIsNotExist_Returns(long orderId)
        {
            // Arrange
            using var databaseService = new NorthwindDatabaseService();
            databaseService.CreateTables();
        ...
        public void UpdateOrderAsync_OrderIsNotExist_ThrownsException(Order order)
        {
            ...
            // Act
            _ = Assert.ThrowsAsync<RepositoryException>(async () => await repository.UpdateOrderAsync(order: order));
